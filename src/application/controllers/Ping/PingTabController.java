...
public class PingTabController {
	@FXML
	...
	private void setServices(Tasks ping, PingOptions options) {
	...
	public void addlisteners(PingOptions options) {
		options.webProperty().addListener((obs,ov,nv) -> {
			if(nv) {
				addToInfo("Changed to Website", "blueStyle");
				isWeb.setText("Website : ");
				conn.setText(options.getWebsite());
			}else {
				addToInfo("Changed to Router", "blueStyle");
				isWeb.setText("Router : ");
				conn.setText(options.getRouter());
			}
		});
	...
	private void setTask(PingOptions options) {
		if(options.isWeb())
			conn.setText(options.getWebsite());
		else
			conn.setText(options.getRouter());
		ping = new Tasks(options);
		setServices(ping,options);
	}
}
